...
function start(){cancelAnimationFrame(frame);reset();state='playing';$('intro').hidden=true;$('results').hidden=true;$('learn').hidden=true;field.focus({preventScroll:true});last=performance.now();frame=requestAnimationFrame(tick);}
function tick(now){if(state!=='playing')return;const dt=Math.max(0,(now-last)/1000);last=now;elapsed=Math.min(30,elapsed+dt);if(elapsed>=30){finish();return;}const {w,h,bw,bh,top}=geometry();const move=(keys.has('ArrowRight')?1:0)-(keys.has('ArrowLeft')?1:0);basketX+=move*dt*1.1;positionBasket();const newLeft=basketX*w-bw/2;const rim=top+bh*.52;
...
window.addEventListener('keydown',e=>{if(state==='playing'&&['ArrowLeft','ArrowRight'].includes(e.key)){e.preventDefault();keys.add(e.key);}});window.addEventListener('keyup',e=>keys.delete(e.key));window.addEventListener('blur',()=>keys.clear());document.addEventListener('visibilitychange',()=>{keys.clear();});
